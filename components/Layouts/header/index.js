...
const Header = ({ header }) => {
  ...
  return (
    <>
      <Head>
        <title>{siteTitle || "NextJ8 Commerce App"}</title>
            ...
            <div className="text-sm font-medium uppercase lg:flex-grow">
              {!isEmpty(headerMenuItems) && headerMenuItems.length
                ? headerMenuItems.map((menuItem) => (
                    <Link href="/categories" key={menuItem?.ID}>
                      <p className="block mt-4 lg:inline-block lg:mt-0 text-black hover:text-black mr-10">
                        {menuItem.title}
                      </p>
                    </Link>
                  ))
